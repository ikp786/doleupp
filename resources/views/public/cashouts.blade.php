...
<div id="collapse-C" class="collapse" role="tabpanel" data-parent="#content"
    ...
    <div class="card-body">
        <form action="{{ route('ready-to-cashout') }}" method="POST">
            @csrf
        ...
        <input class="form-check-input" type="checkbox" name="donations[{{$key}}][cashout]" value="1" id="flexCheckDefault{{$key}}" />
        <label for="flexCheckDefault{{$key}}" style="height: auto;" class="donrs d-block">
            <div class="row">
                ...
                <div class="col-md-6 text-left">
                    <h4 class="mb-0" style="color: #94c21f;">
                        {{--<a href="{{ route('reels.show', ['slug' => $cashout->id]) }}">{{ $cashout->caption ?? '' }}</a>--}}
                        {{ $cashout->caption ?? '' }}
                    </h4>
                    <p>{{ $cashout->category->name ?? '' }} &nbsp; | &nbsp; {{ $cashout->user->name ?? '' }} &nbsp; | &nbsp; <img src="{{ asset('assets/img/eyeb.svg') }}">&nbsp; {{ $cashout->views_count ?? 0 }}</p>
                    {{-- <div class="progress">
        ...
        <div class="row mt-4">
            ...
            <div class="col-md-5 text-end">
                @if(auth()->user()->subscription_ends_at == NULL)
                    <a href="{{ route('subscription') }}" class="btn-get-started d-block text-center">Subscribe Now</a>
                @elseif(auth()->user()->subscription_ends_at < \Carbon\Carbon::now())
                    <a href="{{ route('subscription-renew') }}" class="btn-get-started d-block text-center">Renew Subscription</a>
                @else
