...
@section('title', 'DoleUpp Request - Edit')
        ...
        <form class="modal-content animate" action="{{ route('donation-request') }}" method="post" enctype="multipart/form-data">
            ...
            <div class="container">
                <div class="section-title">
                    <h3>Edit - DoleUpp Request</h3>
                </div>
                <a href="{{ $data->video ?? '' }}" class="ply-video">
                    <img src="{{ $data->thumbnail }}" width="100" height="100" class="rounded"/>
                </a>
                ...
                @enderror
                <textarea placeholder="Caption to Video" rows="2" name="caption">{{ old('caption') ?? $data->caption }}</textarea>
                @error('caption')
                <br><span class="text-danger" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
                @enderror
                <textarea placeholder="Description to Video" rows="3" name="Description">{{ old('Description') ?? $data->Description }}</textarea>
                @error('Description')
                <br><span class="text-danger" role="alert">
                        <strong>{{ $message }}</strong>
                    </span>
                @enderror
                <input disabled class="mt-0" type="text" placeholder="DoleUpp Amount ( Up to ${{ ($donation_amount > 0) ? $donation_amount : 0 }} )" name="donation_amount" value="{{ old('donation_amount') ?? $data->donation_amount }}" required>
